...
We evaluate on a synthetic non-stationary regression task with continuous regime transitions. Ablation results across three seeds show that Δx-based meta-control yields the largest performance gain (avg. −0.155 Seq MSE reduction), while PolicyNet further refines transition behavior by collapsing stable-phase gate entropy to near-deterministic levels (avg. 0.108 vs. 0.951 in Standard MoE). The Nomadic Full model achieves avg. Seq MSE of 0.162 compared to 0.410 for Standard MoE and 0.412 for a fixed baseline. A robustness test under a harder 4-regime / 3-expert underprovisioned condition confirms that the ablation ordering and entropy differentiation signature are preserved (ΔH = +0.437). A parameter-matched baseline experiment confirms that neither capacity increase reduces Seq MSE below 0.409 nor produces ΔH above 0.027, ruling out model capacity as a confound. Task generalization experiments across five variants — nonlinear regime functions, abrupt transitions (steps=2), gradual transitions (steps=24), heavy-tail noise (Student-T, df=2), and their combination — show that Seq MSE advantages over Standard MoE are preserved under nonlinear, abrupt, and heavy-tail conditions, while the entropy differentiation signature degrades under gradual and heavy-tail conditions, revealing robustness boundaries of the current Δx signal design. A recurrent gate baseline (GRU MoE, 26,502 parameters) achieves lower Seq MSE than Nomadic Full (0.105 vs. 0.215) but produces ΔH of only 0.354 compared to Nomadic's 0.755, with Stable Entropy 0.365 vs. 0.176 — confirming that implicit temporal learning improves prediction but does not achieve stable-phase homeomorphic fixation without explicit transition dynamics control. A preliminary LLM experiment transplanting the signal layer onto Gemma-4-E2B shows that ΔH increases from +0.731 (untrained) to +0.984 after PolicyNet training, exceeding the synthetic result, and that LoRA-based expert switching operates across three specialized adapters with a 54.2% switch rate. Real non-stationary time series experiments on ETTh1 (Electricity Transformer Temperature) and Bitcoin price data reveal a prediction pressure boundary condition: on 24-step-ahead ETTh1 prediction, Nomadic Full achieves 13.9% lower Seq MSE than Standard MoE (0.126 vs. 0.147); on 1-step-ahead ETTh1 and Bitcoin, where prediction error remains near-zero or noise-dominated, the Φ signal fails to activate and the MSE advantage disappears — consistent with the gradual-transition and heavy-tail failure modes identified in synthetic task generalization. These results suggest that in synthetic non-stationary settings where temporal context is necessary for regime discrimination, *how* a system transitions between internal representations is as important as *which* representation it selects — a hypothesis that warrants investigation in broader settings.
...
Computing true information gain would require tracking parameter posteriors across experts, which is computationally prohibitive in this setting. The four-term composite is therefore a tractable behavioral proxy: it preserves the directional question of information gain — *is there reason to believe a different internal representation would better account for the current input?* — without requiring explicit posterior inference. The relationship between Φ and the philosophical concept of the Will to Resonance (Φ as orientation toward integrating rather than suppressing Δx) is grounded here: a system that responds to high Φ by exploring alternative routing is, structurally, a system that treats distributional difference as navigational information rather than prediction error to be suppressed.
...
### 4.9 Task Generalization

The experiments in §4.1–4.8 use a controlled synthetic task with linear regime functions, gradual transitions (steps=8), and Gaussian noise. To assess whether the reported gains depend on these specific design choices, we evaluated five task variants while holding all other hyperparameters fixed.
...
**Table 9: Recurrent Gate + Oracle Results (3-seed mean)**
...
**Interpretation.** These results identify a prediction pressure boundary condition for Nomadic's Φ signal. When prediction error is either near-zero (easy task) or near-random (noise-dominated), neither Δx channel generates sufficient routing pressure to differentiate stable and transition phases. This is structurally identical to the gradual-transition failure mode in §4.9: when the signal cannot establish a stable-phase baseline, the system cannot detect departures from it.
...
**Synthetic environment as primary testbed.** The core ablation and robustness results are from controlled synthetic regression tasks. The robustness experiment (§4.4) extends the setting to 4 regimes with random ordering and underprovisioned experts, and §4.5 demonstrates signal transfer to an LLM setting. Task generalization experiments (§4.9) identify three specific structural limitations of the current design. First, gradual transitions (steps=24) cause the transition proportion of the sequence to reach ~50%, preventing the Δx_env signal from establishing stable-phase fixation; all models degrade, and Nomadic Full offers no consistent advantage. Second, heavy-tail noise (Student-T, df=2) introduces episodic spikes in Δx_err that the error-EMA baseline cannot distinguish from genuine regime transitions, corrupting the Φ signal and eliminating entropy differentiation while leaving the MSE ranking intact. Third, the combined condition (nonlinear + abrupt + heavy-tail) exceeds the current architecture's stability boundary, with PolicyNet training destabilized by extreme output scales and noise. These findings motivate concrete improvements: a transition-ratio-aware Φ scaling for gradual settings, a robust estimator (e.g., Huber-loss-based EMA) for Δx_err under heavy-tail noise, and output normalization with gradient clipping for combined stress conditions. The synthetic tasks use clean periodic transitions and Gaussian regime sampling that do not reflect the noise and non-Markovian structure of real environments. We make no claim that current hyperparameters transfer to real-world settings without adjustment.

**Prediction pressure boundary in real time series.** Real-world experiments (§4.11) reveal a further limitation: the Φ signal requires sufficient prediction pressure to activate. On ETTh1 1-step-ahead prediction, MSE collapses to ≈0.007 within 50 epochs and both Δx channels remain near-zero thereafter, producing uniform routing (Stable H ≈ log(3)) and no MSE advantage. On Bitcoin, near-random prediction error (MSE ≈ 1.0) provides no stable EMA baseline for Δx_err to detect departures from, replicating the synthetic heavy-tail failure mode in a real-world setting. The 24-step ETTh1 result (13.9% MSE improvement) represents a partial activation regime: prediction difficulty is sufficient to sustain Δx_err, but homeomorphic fixation is not achieved (ΔH ≈ −0.009 vs. synthetic +0.755). This suggests that the current Δx_err formulation — a relative measure comparing current EMA error to a slower baseline — requires a minimum error magnitude and a minimum stable-run duration to function as designed. Tasks where the model converges too quickly or where error is structurally irreducible will not benefit from the Φ mechanism in its current form.
...
**Oracle upper bound is not absolute.** The regime-specialist Oracle in §4.10 trains each expert on stable-phase data only and routes via dominant regime labels. Because transition batches carry linearly-interpolated labels that no single regime expert can optimally approximate, Oracle MSE (0.330) exceeds Nomadic Full's (0.215). A stronger Oracle — one that uses α-weighted expert mixtures for transition batches — would provide a tighter upper bound and is deferred to future work.
