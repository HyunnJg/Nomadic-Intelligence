...
- **Phi\_EMA** (baseline): 
  $\Phi = \tanh\!\left(s_{\mathrm{env}} \cdot \Delta x^{\mathrm{env}} + s_{\mathrm{err}} \cdot \Delta x^{\mathrm{err}} + s_{\mathrm{exp}} \cdot \mathcal{L}_{\mathrm{task}} + s_{\mathrm{gap}} \cdot \mathrm{gap}_t\right)$

- **Phi\_JSD**: 
  $\Phi = \tanh\!\left(\alpha \cdot \mathrm{JSD}\!\left(\bar{g}_t \,\|\, \bar{g}_{t-1}\right)\right)$, Jensen–Shannon divergence between consecutive batch-mean gate distributions

- **Phi\_KL**: 
  $\Phi = \tanh\!\left(\alpha \cdot \mathrm{KL}\!\left(\bar{g}_t \,\|\, \bar{g}_{t-1}\right)\right)$, asymmetric forward KL divergence

- **Phi\_Switch**: 
  $\Phi = p_t^{\mathrm{switch}}$, PolicyNet switch head output used directly as $\Phi$ (end-to-end)

- **Phi\_JSD\_v2**: 
  $\Phi = \tanh\!\left(
  s_{\mathrm{div}} \cdot \operatorname{std}_i\!\left[\mathrm{JSD}\!\left(g_i \,\|\, \bar{g}_t\right)\right]
  + 
  s_{\mathrm{ema}} \cdot \mathrm{EMA}\!\left(
  \operatorname{mean}_i\!\left[\mathrm{JSD}\!\left(g_i \,\|\, \bar{g}_t\right)\right]
  \right)
  \right)$, intra-batch routing heterogeneity
