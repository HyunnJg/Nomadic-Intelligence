...
def build_config_from_yaml(yaml_dict: dict) -> Config:
    ...
    cfg = Config(
        ...
        beta_phi=loss.get("beta_phi", 0.05),
...
def compute_phi_reward(warm_mse: torch.Tensor, adapted_mse: torch.Tensor) -> torch.Tensor:
    """
    Partial proxy for Phi (Will to Resonance) — specifically the R_sync component.
    Phi = alpha * R_sync - beta * P_dogma + gamma * R_nomad
    P_dogma is handled by compute_dogma_penalty()
    R_nomad is handled by compute_nomad_bonus() and DwellTimeRegularizer
    R_sync proxy: reward when delta_x integration actually improves prediction.
    """
    return torch.relu(warm_mse.detach() - adapted_mse)
...
def train_nomadic(cfg: Config, X_train, Y_train, R_train, X_test, Y_test, R_test, phase_tags_test):
    ...
    for epoch in range(cfg.epochs):
        ...
        for xb, yb, rb in iterate_sequence_minibatches(X_train, Y_train, R_train, cfg.phase_batch_size):
            ...
            phi_reward = compute_phi_reward(warm_mse, mse_loss)
