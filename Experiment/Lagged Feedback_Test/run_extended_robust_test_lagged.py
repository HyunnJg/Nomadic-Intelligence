...
class Config:
    ...
    # extended experiment
    num_regimes: int = 4           # 3 = original A/B/C, 4 = add D
    random_regime_order: bool = True  # False = fixed A→B→C(→D) cycle
...
def build_config_from_yaml(yaml_dict: dict) -> Config:
    ...
    cfg = Config(
        ...
        num_regimes=data.get("num_regimes", 4),
        random_regime_order=data.get("random_regime_order", True),
    )
    return cfg
...
def generate_phase_sequence(cfg: Config, cycles: int, device: str = "cpu"):
    ...
    If cfg.num_regimes == 4: uses regimes A, B, C, D.
    If cfg.random_regime_order: shuffles regime order each cycle independently,
      guaranteeing no two consecutive cycles start with the same regime
      and that all regimes appear in every cycle.
    Otherwise: fixed cyclic order (A→B→C→D or A→B→C).
    ...
    rng = np.random.default_rng(cfg.seed + 99)  # separate rng so data is reproducible
    prev_last_regime = None

    for cycle_idx in range(cycles):
        if cfg.random_regime_order:
            # shuffle until first element differs from previous cycle's last element
            order = list(active_regimes)
            for _ in range(200):
                rng.shuffle(order)
                if prev_last_regime is None or order[0] != prev_last_regime:
                    break
            cycle_regimes = order
        else:
...
def regimewise_usage(gate_probs: torch.Tensor, regime_ids: torch.Tensor, num_experts: int, num_regimes: int = 4) -> Dict[str, np.ndarray]:
    usage = {}
    top1 = gate_probs.argmax(dim=-1)
...
def mse_by_regime(y_true: torch.Tensor, y_pred: torch.Tensor, regime_ids: torch.Tensor, num_regimes: int = 4) -> Dict[str, float]:
    result = {}
    for rid in range(num_regimes):
        mask = regime_ids == rid
        regime_name = ID_TO_REGIME[rid]
...
def infer_regime_to_expert(usage: Dict[str, np.ndarray], num_regimes: int = 4) -> Dict[str, int]:
    mapping = {}
    for rid in range(num_regimes):
        regime = ID_TO_REGIME[rid]
        if regime in usage:
...
def evaluate_fixed(model: nn.Module, X: torch.Tensor, Y: torch.Tensor, R: torch.Tensor, cfg: Config):
    model.eval()
    with torch.no_grad():
        y_pred = model(X)
        total_mse = F.mse_loss(y_pred, Y).item()
        per_regime = mse_by_regime(Y, y_pred, R, cfg.num_regimes)
...
def evaluate_standard_moe(model: StandardMoE, X: torch.Tensor, Y: torch.Tensor, R: torch.Tensor, cfg: Config):
    ...
    with torch.no_grad():
        ...
        total_mse = F.mse_loss(y_pred, Y).item()
        per_regime = mse_by_regime(Y, y_pred, R, cfg.num_regimes)
        usage = regimewise_usage(gate_probs, R, cfg.num_experts, cfg.num_regimes)
...
def evaluate_nomadic_sequence_dynamics(model: NomadicMoE, X: torch.Tensor, Y: torch.Tensor, R: torch.Tensor, phase_tags: List[str], cfg: Config):
    ...
    usage = regimewise_usage(G, R, cfg.num_experts, cfg.num_regimes)
    regime_to_expert = infer_regime_to_expert(usage, cfg.num_regimes)
...
def train_fixed(cfg: Config, X_train, Y_train, R_train, X_test, Y_test, R_test):
    ...
    for epoch in range(cfg.epochs):
        ...
        test_mse, _ = evaluate_fixed(model, X_test, Y_test, R_test, cfg)
...
def evaluate_nomadic_no_policy_sequence(
    ...
    latencies = compute_switch_latency(
        ...
        np.array(batch_top1),
        infer_regime_to_expert(usage, cfg.num_regimes),
    )
...
def train_nomadic_no_policy(cfg: Config, X_train, Y_train, R_train, X_test, Y_test, R_test, phase_tags_test):
    ...
    for epoch in range(cfg.epochs):
        ...
        epoch_loss = 0.0
        n_batches = 0
        prev_yb = torch.zeros((cfg.phase_batch_size, cfg.output_dim), device=cfg.device)  # [Lagged] buffer
        ...
        for xb, yb, rb in iterate_sequence_minibatches(X_train, Y_train, R_train, cfg.phase_batch_size):
            ...
            # [Lagged] warm_mse는 이전 배치 타깃(prev_yb)으로 계산 — 인과율 준수
            with torch.no_grad():
                zero_delta = torch.zeros((xb.size(0), 1), device=cfg.device)
            ...
            # [Lagged] explanation signal도 prev_yb 기준
            explanation_error, best_expert_gap = compute_explanation_signals(
                y_true=prev_yb, y_hat=probe_y,
                expert_outputs=probe_expert_outputs, gate_probs=probe_gate_probs,
            )
            phi_signal = compute_phi_signal(
                delta_env_scalar=de, delta_err_scalar=derr,
            ...
            # [Lagged] backward는 현재 yb로 — 그라디언트는 실제 타깃 기준
            mse_loss = F.mse_loss(y_hat, yb)
            ...
            _, sep_loss, cons_loss, _, _ = compute_regime_gate_stats(
                gate_probs=final_routing, regime_ids=rb, num_regimes=cfg.num_regimes,
            )
            ...
            epoch_loss += total_loss.item()
            n_batches += 1
            prev_yb = yb.detach()  # [Lagged] 다음 배치를 위해 현재 타깃을 저장
...
def train_nomadic(cfg: Config, X_train, Y_train, R_train, X_test, Y_test, R_test, phase_tags_test):
    ...
    for epoch in range(cfg.epochs):
        ...
        for xb, yb, rb in iterate_sequence_minibatches(X_train, Y_train, R_train, cfg.phase_batch_size):
            ...
            # explanation signals — [Lagged] probe도 prev_yb 기준
            with torch.no_grad():
                probe_y, probe_gate_probs, _, probe_expert_outputs = model(
                    xb, delta_hybrid, delta_err_tensor, cfg.temperature, hard=False
                )
            ...
            # teacher targets (hybrid heuristic supervision) — [Lagged] prev_yb 기준
...
def plot_usage_bars(usage: Dict[str, np.ndarray], save_path: str, title: str):
    regimes = [r for r in ["A", "B", "C", "D"] if r in usage]
    num_experts = len(next(iter(usage.values())))
    x = np.arange(len(regimes))
    width = max(0.15, 0.8 / num_experts)

    plt.figure(figsize=(9, 5))
    for e in range(num_experts):
        vals = [usage[r][e] for r in regimes]
        offset = (e - num_experts / 2 + 0.5) * width
        plt.bar(x + offset, vals, width=width, label=f"Expert {e}")
...
def plot_gate_heatmap(usage: Dict[str, np.ndarray], save_path: str):
    ...
    plt.figure(figsize=(7, max(4, len(regimes))))
...
def print_report(
    ...
    for regime in sorted(standard_moe_usage.keys()):
    ...
    for regime in sorted(nomadic_usage.keys()):
    ...
    # Expert sharing analysis (relevant when num_regimes > num_experts)
    top1_assignments = {r: int(np.argmax(arr)) for r, arr in nomadic_usage.items()}
    from collections import Counter
    expert_load = Counter(top1_assignments.values())
    shared = [e for e, cnt in expert_load.items() if cnt > 1]
    if shared:
        print(f"\n[Expert Sharing]  (regimes={len(nomadic_usage)}, experts={len(arr)})")
        for exp_id in sorted(shared):
            regimes_on_exp = [r for r, e in top1_assignments.items() if e == exp_id]
            print(f"  Expert {exp_id} shared by: {', '.join(sorted(regimes_on_exp))}")
...
def main():
    ...
    active_regimes = ["A", "B", "C", "D"][:cfg.num_regimes]
    print(f"Experiment: {cfg.num_regimes} regimes {active_regimes}  |  "
          f"random_order={cfg.random_regime_order}  |  "
          f"num_experts={cfg.num_experts}  |  seed={cfg.seed}")
    if cfg.num_regimes > cfg.num_experts:
        print(f"  *** Underprovisioned: {cfg.num_regimes} regimes / {cfg.num_experts} experts "
              f"— expert sharing expected ***")
    ...
    fixed_total_mse, fixed_per_regime = evaluate_fixed(fixed_model, X_test, Y_test, R_test, cfg)
