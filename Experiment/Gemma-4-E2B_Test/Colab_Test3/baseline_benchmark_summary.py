...
PROMPTS = {
    ...
    "transition": [
        ...
        "과거에는 옳았던 것이 이제는 틀렸다. 왜냐하면",
        "Although everything seemed fine, suddenly",
        "상황이 급격하게 변하기 시작했고",
        "기존 질서는 무너지고 새로운 규칙이 등장했다"
    ],
    "creative": [
        "만약 AI가 감정을 가진다면",
        "미래의 인공지능은 생물학적 한계를",
        "시간이 거꾸로 흐른다면",
        "인류가 완전히 다른 감각기관을 가졌다면",
        "의식이 디지털로 이전된다면"
    ]
}
...
# ============================================================
# STEP 11: 벤치마크 실행 및 결과 패키징
# + multi-run(n=3) + perplexity 저장
# ============================================================
def package_result(model_name, prompt_type, prompt, run_id, res):
    metrics = summarize_text_metrics(res["text"])
    return {
        "model": model_name,
        ...
        "mean_log_prob": res["mean_log_prob"],
...
print("\n🚀 [작전 개시] 3단계 베이스라인 벤치마크 평가를 시작합니다...")
...
for ptype, plist in PROMPTS.items():
    ...
    for prompt in plist:
        for run_id in range(NUM_RUNS):
            print(f"\n[Prompt] {prompt} | run {run_id+1}/{NUM_RUNS}")

            # 1. Vanilla
            res_v = generate_vanilla(prompt)
            all_results.append(package_result("1_Vanilla", ptype, prompt, run_id + 1, res_v))
            print("  ✅ Vanilla 완료")

            # 2. Dynamic Temp
            res_d = generate_dynamic_temp(prompt)
            all_results.append(package_result("2_DynamicTemp", ptype, prompt, run_id + 1, res_d))
            print("  ✅ Dynamic Temp 완료")

            # 3. Full Nomadic
            res_n = generate_nomadic_full(prompt, expert_paths, policy_net)
            all_results.append(package_result("3_NomadicFull", ptype, prompt, run_id + 1, res_n))
            print("  ✅ Nomadic Full 완료")

# 결과 저장
benchmark_path = os.path.join(RUN_DIR, 'baseline_benchmark_results.json')
with open(benchmark_path, 'w', encoding='utf-8') as f:
    json.dump(all_results, f, ensure_ascii=False, indent=2)

print(f"\n💾 벤치마크 결과 저장 완료: {benchmark_path}")

# ============================================================
# STEP 12: 콘솔 요약 출력 (Pandas)
# ============================================================
import pandas as pd
...
for row in all_results:
    summary[row["model"]][row["prompt_type"]].append(row)

summary_rows = []
for model_name, type_dict in summary.items():
    for ptype, rows in type_dict.items():
        summary_rows.append({
            "Model": model_name,
            "Prompt Type": ptype,
            "Entropy": np.mean([r["mean_entropy"] for r in rows]),
            "Mean LogProb": np.mean([r["mean_log_prob"] for r in rows]),
            "Perplexity": np.mean([r["perplexity"] for r in rows]),
            "Distinct-2": np.mean([r["distinct_2"] for r in rows]),
            "Repetition(Bi)": np.mean([r["repeat_rate_bigram"] for r in rows]),
        })

df_summary = pd.DataFrame(summary_rows).sort_values(["Prompt Type", "Model"]).reset_index(drop=True)
print("\n📊 [벤치마크 최종 요약 결과]")
print(df_summary.to_string(index=False))

summary_csv_path = os.path.join(RUN_DIR, 'baseline_benchmark_summary.csv')
