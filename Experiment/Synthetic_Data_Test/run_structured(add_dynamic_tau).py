...
class HybridDeltaTracker:
    ...
    Added:
      sigma2_delta = rolling variance of recent delta_env
      dynamic_tau  = environment-aware dwell horizon
                     low sigma2  -> tau large  (fixation allowed)
                     high sigma2 -> tau small  (nomadic pressure)
    """
    def __init__(
        self,
        ema_decay: float = 0.8,
    ...
    def compute_dynamic_tau(self, sigma2_delta: float) -> float:
        """
        Monotonically decreasing in sigma2_delta.
        sigma2=0   -> tau_max  (fully stable: maximum fixation allowed)
        sigma2->inf -> tau_min  (volatile: minimum dwell, nomadic pressure)
        """
...
class DwellTimeRegularizer:
    ...
    Loss integration: total_loss = ... - dwell_bonus
    So:
    ...
    Regime logic:
      stable phase  (dwell_count <= tau_capacity):
          encourage fixation -> entropy should DECREASE
          -> return negative value so loss increases and entropy is penalized
          -> dwell_bonus = -penalty * entropy

      transition phase (dwell_count > tau_capacity):
          encourage switching -> entropy should INCREASE
          -> return positive value so loss decreases and entropy is rewarded
          -> dwell_bonus = +excess_weight * entropy

    tau_capacity:
      dynamic tau from HybridDeltaTracker (large when stable, small when volatile)
      falls back to tau_k_min when use_dynamic_tau=False
    """
    ...
    def compute(self, gate_probs: torch.Tensor, tau_dynamic: float = None) -> torch.Tensor:
        ...
        if self.dwell_count <= tau_capacity:
            # Stable phase: penalize entropy (push toward fixation)
            # Negative return -> loss increases -> entropy decreases
            return -self.penalty * entropy
        else:
            # Transition phase: reward entropy (push toward switching)
            # Positive return -> loss decreases -> entropy increases
