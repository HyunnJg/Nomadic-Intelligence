# -*- coding: utf-8 -*-
"""scenario_D_phi_comparison_A100.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1WsUCkq3K_gxlf7_rm4f3wHKEs9FHuWju

# Nomadic Intelligence — Scenario D
## Φ (Switching Pressure) 4-Variant Comparative Experiment

**목적**: 현재 Nomadic Full 모델의 Φ는 hand-crafted composite 신호
(`tanh(s_env·Δx_env + s_err·Δx_err + s_exp·L_task + s_gap·gap_t)`).
Theory_and_Axioms.md §5.3이 인정하듯 *'formal information-theoretic derivation 없음'* 상태.

이 실험은 4종의 Φ 구현체를 동일 NomadicMoE 아키텍처에 플러그인으로 교체하여
**Seq MSE, ΔH(transition−stable entropy), Switch Latency** 기준으로 직접 비교한다.

| Variant | 수식 | 특징 |
|---|---|---|
| `Phi_EMA` | 현재 구현 (baseline) | composite heuristic |
| `Phi_JSD` | JSD(g_t ‖ g_{t-1}) | gate 분포 이동량 직접 측정 |
| `Phi_KL` | KL(g_t ‖ g_{t-1}) | asymmetric, 더 sensitive |
| `Phi_Switch` | PolicyNet switch_prob 자체 | end-to-end learned Φ |

**기대 귀결**:
- Phi_JSD/KL > Phi_EMA → Φ의 information-theoretic 근거 강화 (논문 §5.3 Limitation 해소)
- Phi_Switch best → PolicyNet이 Φ를 내재화함 (RL 도입의 근거)
- Phi_EMA best → 현재 heuristic이 실용적으로 충분 (현 상태 유지 정당화)

**Seeds**: 42, 123, 456 (기존 논문과 동일)

**실험환경**: A100 최적화 버전 (원본 run_structured.py 하이퍼파라미터 완전 보존)

---
**최적화 요약** (실험 결과에 영향 없는 엔지니어링 변경만):
1. `compute_load_balancing_loss` — Python for loop → `bincount` 벡터화 (GPU sync 제거)
2. warm pass + probe pass 통합 → forward 3회 → 2회 (수학적으로 동등)
3. `HybridDeltaTracker` / `DwellTimeRegularizer` 재사용 (epoch마다 생성 → reset만)
4. 데이터셋 seed별 사전 생성 후 variant 루프에서 재사용
5. Colab 메타데이터 A100으로 업데이트
"""
...
PHI_VARIANTS = ['Phi_EMA', 'Phi_JSD', 'Phi_KL', 'Phi_Switch']
SEEDS = [42, 123, 456]
...
def make_phi(name: str) -> PhiComputer:
    return {'Phi_EMA': PhiEMA, 'Phi_JSD': PhiJSD,
            'Phi_KL': PhiKL, 'Phi_Switch': PhiSwitch}[name]()
...
LINESTYLES = {'Phi_EMA': '-', 'Phi_JSD': '--', 'Phi_KL': '-.', 'Phi_Switch': ':'}
...
# ============================================================
# STEP 15: 이론적 해석 자동 출력
# ============================================================

print('\n' + '='*72)
print('SCENARIO D — THEORETICAL INTERPRETATION')
print('='*72)
...
if best_dh_name in ('Phi_JSD', 'Phi_KL'):
    print("""
[RESULT: Information-geometric Φ wins on ΔH]
  → Φ의 information-theoretic 근거가 실증적으로 지지됨.
  → Theory §Φ에 다음 문장 추가 가능:
     'JSD/KL 기반 Φ가 heuristic composite보다 높은 ΔH를 달성,
      suggesting Φ ≈ gate distribution divergence 해석의 타당성.'
  → arXiv v2에서 §5.3 Limitation의 Φ theoretical grounding 항목 해소 가능.
""")
elif best_dh_name == 'Phi_Switch':
    print("""
[RESULT: PolicyNet-learned Φ wins on ΔH]
  → PolicyNet이 switching pressure를 내재화함.
  → RL 도입의 강한 근거: PolicyNet이 이미 Φ를 학습하고 있다면
     reward signal로 강화하는 것이 자연스러운 다음 단계.
  → Theory §Φ에 'Φ는 학습 가능하다'는 명제 추가 정당화.
""")
else:
    print("""
[RESULT: EMA composite Φ remains competitive]
  → 현재 heuristic이 실용적으로 충분함을 실험적으로 확인.
  → Theory §Φ: 'formal derivation 부재'는 여전하나,
     'empirically robust against information-geometric alternatives' 추가 가능.
  → 향후 방향: gap_t term이 실질적 공헌자일 가능성 — ablation 추천.
""")

print()
print('Saved figures: scenario_D_learning_curves.png, scenario_D_delta_H.png,')
