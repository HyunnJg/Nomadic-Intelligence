# -*- coding: utf-8 -*-
"""scenario_D_v5_jsd_v2_only.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1_V_cpjIp03Opwtd9eVMHAjom-tgPEZOv

# Nomadic Intelligence — Scenario D (v2)
## Φ Variant Comparison: Phi_JSD_v2 단독 실험 (v5)

**목적**: s_div 4.0 복원 + detach() 추가 후 Phi_JSD_v2 단독 재실험
- v3 대비 변경: s_div 8.0→4.0, std_term/mean_term에 .detach() 명시
- v4 GPU 최적화(rolling variance, 벡터화 diversity loss)는 유지
...
**v5 수정 사항**:
- `PhiJSD_v2.__init__`: s_div 8.0 → 4.0 (v3 원복)
- `PhiJSD_v2.compute`: std_term, mean_term에 `.detach()` 추가 (gradient graph 차단)
"""
...
PHI_VARIANTS = ['Phi_JSD_v2']  # v5: 단독 실험
SEEDS = [42, 123, 456]
...
class PhiJSD_v2(PhiComputer):
    ...
    """
    def __init__(self, s_div: float = 4.0,  # v5: 8.0→4.0 복원 (v3 기준)
                 s_ema: float = 3.0,
    ...
    def compute(self, delta_env, delta_err, explanation_error, best_expert_gap,
        ...
        std_term  = per_jsd.std().detach()   # v5: detach — gradient graph 차단
        mean_term = per_jsd.mean().detach()  # v5: detach — gradient graph 차단

        # EMA 업데이트 — GPU tensor로 유지
        if self.ema_mean_jsd is None:
            self.ema_mean_jsd = mean_term.detach()
        else:
            self.ema_mean_jsd = (self.ema_decay * self.ema_mean_jsd
                                 + (1.0 - self.ema_decay) * mean_term.detach())
...
# ============================================================
# STEP 10: 시각화 — Phi_JSD_v2 단독 학습 곡선
# ============================================================

COLORS     = {'Phi_JSD_v2': '#9467BD'}
LINESTYLES = {'Phi_JSD_v2': (0, (3, 1, 1, 1))}

fig, axes = plt.subplots(2, 2, figsize=(14, 9))
fig.suptitle('Scenario D v5: Phi_JSD_v2 단독 실험\n(3-seed mean ± std, shaded)',
...
for metric_key, ax, ylabel, lower_is_better in metric_specs:
    for phi_name in PHI_VARIANTS:
        ...
        ax.fill_between(epochs, mean_c - std_c, mean_c + std_c,
                         color=COLORS[phi_name], alpha=0.15)
    ax.set_xlabel('Epoch'); ax.set_ylabel(ylabel)
    ax.legend(fontsize=9); ax.grid(alpha=0.3); ax.set_title(ylabel)

plt.tight_layout()
plt.savefig('scenario_D_v5_learning_curves.png', dpi=150, bbox_inches='tight')
plt.show()
print('Figure saved: scenario_D_v5_learning_curves.png')

# ============================================================
# STEP 11: ΔH trajectory (Phi_JSD_v2 단독)
# ============================================================

fig, ax = plt.subplots(figsize=(9, 4))
fig.suptitle('Scenario D v5: ΔH Trajectory — Phi_JSD_v2', fontsize=12, fontweight='bold')

curves = np.array([
    np.array(all_results['Phi_JSD_v2'][s]['test_transition_entropy'])
    - np.array(all_results['Phi_JSD_v2'][s]['test_stable_entropy'])
    for s in SEEDS
])
...
for s in SEEDS:
    dh_s = (np.array(all_results['Phi_JSD_v2'][s]['test_transition_entropy'])
            - np.array(all_results['Phi_JSD_v2'][s]['test_stable_entropy']))
    ax.plot(epochs, dh_s, color='#9467BD', alpha=0.3, linewidth=1.0, ls='--', label=f'seed {s}')

ax.plot(epochs, mean_c, color='#9467BD', linewidth=2.2, label='mean')
ax.fill_between(epochs, mean_c - std_c, mean_c + std_c, color='#9467BD', alpha=0.15)
ax.axhline(0, color='gray', lw=0.8, ls='--')

# v3 참조선
ax.axhline(0.4436, color='orange', lw=1.2, ls=':', label='v3 mean ΔH=0.444')

ax.set_xlabel('Epoch'); ax.set_ylabel('ΔH')
ax.legend(fontsize=9); ax.grid(alpha=0.3)

# 최종값 출력
final_dh = [all_results['Phi_JSD_v2'][s]['test_transition_entropy'][-1]
            - all_results['Phi_JSD_v2'][s]['test_stable_entropy'][-1] for s in SEEDS]
print(f'\nFinal ΔH per seed: {[f"{v:.4f}" for v in final_dh]}')
print(f'Mean ΔH: {np.mean(final_dh):.4f} ± {np.std(final_dh):.4f}')
print(f'v3 기준 ΔH: 0.4436  →  diff: {np.mean(final_dh)-0.4436:+.4f}')

plt.tight_layout()
plt.savefig('scenario_D_v5_delta_H.png', dpi=150, bbox_inches='tight')
plt.show()
print('Figure saved: scenario_D_v5_delta_H.png')

# ============================================================
# STEP 12: Φ 신호 trace — Phi_JSD_v2 (seed별)
# ============================================================

fig, axes = plt.subplots(1, 3, figsize=(15, 4), sharey=True)
fig.suptitle('Scenario D v5: Φ Signal Trace — Phi_JSD_v2', fontsize=11, fontweight='bold')

for ax, seed in zip(axes, SEEDS):
    phi_curve = np.array(all_results['Phi_JSD_v2'][seed]['phi_values'])
    ax.plot(phi_curve, color='#9467BD', linewidth=1.5)
...
plt.tight_layout()
plt.savefig('scenario_D_v5_phi_trace.png', dpi=150, bbox_inches='tight')
plt.show()
print('Figure saved: scenario_D_v5_phi_trace.png')

# ============================================================
# STEP 13: Per-seed 상세 결과 테이블 (v5)
# ============================================================

rows = []
for seed in SEEDS:
    logs = all_results['Phi_JSD_v2'][seed]
...
print('\n=== v5 Per-seed Results (Phi_JSD_v2) ===')
print(df_detail.to_string(float_format=lambda x: f'{x:.4f}', index=False))

print('\n=== Aggregated ===')
mse_v  = df_detail['Seq MSE'].values
dh_v   = df_detail['ΔH'].values
lat_v  = df_detail['Switch Lat'].values
print(f'  Seq MSE : {mse_v.mean():.4f} ± {mse_v.std():.4f}')
print(f'  ΔH      : {dh_v.mean():.4f} ± {dh_v.std():.4f}')
print(f'  Latency : {lat_v.mean():.4f} ± {lat_v.std():.4f}')

print('\n=== 비교 (v3 기준) ===')
print(f'  v3 Seq MSE : 0.2759 ± 0.0299  →  diff: {mse_v.mean()-0.2759:+.4f}')
print(f'  v3 ΔH      : 0.4436 ± 0.0483  →  diff: {dh_v.mean()-0.4436:+.4f}')
print()
print('  원인 판별:')
print(f'    ΔH v3→v5 diff: {dh_v.mean()-0.4436:+.4f}')
print(f'    예상: s_div 복원+detach로 v3 수준 회복 시 |diff| < 0.05')
'''
v4 비교도 참고용으로 출력
'''
print('  v4 ΔH      : 0.1168 ± 0.0270  (s_div=8.0, no detach)')
print(f'  v5 vs v4 diff: {dh_v.mean()-0.1168:+.4f}  → 어느 쪽이 주원인인지 확인')

# ============================================================
# STEP 14: 결과 해석 — v3/v4/v5 비교 자동 출력
# ============================================================

dh_vals  = [all_results['Phi_JSD_v2'][s]['test_transition_entropy'][-1]
            - all_results['Phi_JSD_v2'][s]['test_stable_entropy'][-1] for s in SEEDS]
mse_vals = [all_results['Phi_JSD_v2'][s]['test_seq_mse'][-1] for s in SEEDS]

v5_dh  = np.mean(dh_vals)
v5_mse = np.mean(mse_vals)
v3_dh, v4_dh   = 0.4436, 0.1168
v3_mse, v4_mse = 0.2759, 0.2374

print('=' * 60)
print('Phi_JSD_v2: v3 / v4 / v5 비교')
print('=' * 60)
print(f'{"":20s} {"ΔH":>8} {"Seq MSE":>10} {"s_div":>7} {"detach":>8}')
print(f'{"v3 (원본)":20s} {v3_dh:>8.4f} {v3_mse:>10.4f} {"4.0":>7} {"no":>8}')
print(f'{"v4 (변경)":20s} {v4_dh:>8.4f} {v4_mse:>10.4f} {"8.0":>7} {"no":>8}')
print(f'{"v5 (수정)":20s} {v5_dh:>8.4f} {v5_mse:>10.4f} {"4.0":>7} {"yes":>8}')
print('-' * 60)

delta_from_v3 = v5_dh - v3_dh
delta_from_v4 = v5_dh - v4_dh

print(f'\nv5 - v3 ΔH = {delta_from_v3:+.4f}')
print(f'v5 - v4 ΔH = {delta_from_v4:+.4f}')
print()

if abs(delta_from_v3) < 0.05:
    print('[판정] v5 ≈ v3 → 두 수정(s_div 복원 + detach)으로 완전 회복.')
    print('      s_div=8.0 과대 + detach 누락의 복합 원인이었음.')
elif v5_dh > v4_dh and abs(delta_from_v3) >= 0.05:
    print('[판정] v5 > v4이나 v3에 미달 → 부분 회복.')
    print('      detach 혹은 s_div 중 하나가 주원인. 추가 분리 실험 필요.')
elif v5_dh <= v4_dh:
    print('[판정] v5 ≤ v4 → 예상치 못한 결과. 다른 원인 검토 필요.')
...
SAVE_DIR = '/content/scenario_D_v5_results'
os.makedirs(SAVE_DIR, exist_ok=True)

df_detail.to_csv(os.path.join(SAVE_DIR, 'results_per_seed_v5.csv'), index=False)

serializable = {'Phi_JSD_v2': {}}
for seed in SEEDS:
    serializable['Phi_JSD_v2'][str(seed)] = {
        k: [float(x) for x in v]
        for k, v in all_results['Phi_JSD_v2'][seed].items()
    }

with open(os.path.join(SAVE_DIR, 'all_logs_v5.json'), 'w') as f:
    json.dump(serializable, f, indent=2)

for fname in ['scenario_D_v5_learning_curves.png',
              'scenario_D_v5_delta_H.png',
              'scenario_D_v5_phi_trace.png']:
    src_path = f'/content/{fname}'
    if os.path.exists(src_path):
        shutil.copy(src_path, os.path.join(SAVE_DIR, fname))
