# -*- coding: utf-8 -*-
"""recurrent_oracle_experiment.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1E5Hl0O2z4U7sru2mYS8Lj8RrDUjzp4lw

# Nomadic Intelligence — §4.10 Recurrent Gate + Oracle Baseline
...
| **Oracle** | per-batch best-expert hard routing (upper bound) | — |
...
class GRUMoE(nn.Module):
    ...
    def forward(self, x, h, temperature=1.0, hard=False):
        ...
        logits = self.gate_head(h_new)           # [1, K]
...
def eval_oracle(experts,X,Y,R,phase_tags,cfg):
    """
    Oracle: per-batch best-expert hard routing.
    For each batch, run all experts, pick the one with lowest MSE, hard-route.
    No training. Uses pre-trained Nomadic experts (or can be standalone).
    Upper bound on achievable MSE with the given expert set.
    """
    [e.eval() for e in experts]
    all_y,tags,ents,top1s,regs=[],[],[],[],[]
    with torch.no_grad():
        for bi,(xb,yb,rb) in enumerate(iterate_minibatches(X,Y,R,cfg.phase_batch_size)):
            # compute MSE per expert
            per_mse=[F.mse_loss(e(xb),yb).item() for e in experts]
            best=int(np.argmin(per_mse))
            yh=experts[best](xb)
            K=len(experts)
            gp=F.one_hot(torch.full((xb.size(0),),best,dtype=torch.long,device=cfg.device),K).float()
            all_y.append(yh)
            tags.append(phase_tags[bi*cfg.phase_batch_size])
            ents.append(gate_entropy(gp).mean().item())   # always 0 (hard)
            top1s.append(best)
            regs.append(ID_TO_REGIME[int(rb[0].item())])
    seq_mse=F.mse_loss(torch.cat(all_y),Y).item()
    r2e={r:i for i,r in enumerate(['A','B','C'])}  # trivial for oracle
    lats=compute_switch_latency(regs,np.array(top1s),r2e)
    sh=[e for t,e in zip(tags,ents) if t.startswith('stable_')]
    th=[e for t,e in zip(tags,ents) if t.startswith('transition_')]
    return seq_mse,{'sh':float(np.mean(sh)) if sh else 0.0,
                    'th':float(np.mean(th)) if th else 0.0,
                    'dh':float(np.mean(th)-np.mean(sh)) if(sh and th) else 0.0,
                    'lat':float(np.mean(lats)) if lats else float('nan')}
...
def train_gru(cfg,Xtr,Ytr,Rtr,Xte,Yte,Rte,tags_te):
    ...
    """
    m=GRUMoE(cfg.input_dim,cfg.hidden_dim,cfg.output_dim,cfg.num_experts).to(cfg.device)
    opt=torch.optim.Adam(m.parameters(),lr=cfg.lr,weight_decay=cfg.weight_decay)
    mse_log,dyn_log=[],[]
    for ep in range(cfg.epochs):
        m.train()
...
for seed in SEEDS:
    ...
    print('--- Oracle (using Nomadic experts) ---')
    # Oracle은 Nomadic의 학습된 expert들을 재사용 — 동일 expert capacity 조건
    oracle_mse,oracle_dyn=eval_oracle(list(nm.experts),Xte,Yte,Rte,tags_te,cfg)
    all_results['Oracle'][seed]={'mse_log':[oracle_mse],'dyn_log':[oracle_dyn]}
    print(f'  [Oracle]  MSE={oracle_mse:.4f}  (upper bound with Nomadic experts)')
...
SAVE_DIR='/content/recurrent_oracle_results'
